...
function adicionarItemOrcamento(produtoId = "", quantidade = 1, valorSalvo = null) {
  ...
  preco.addEventListener("input", () => config.recalcularTotal());
  quantidadeInput.addEventListener("input", () => config.recalcularTotal());
  buscaProduto.addEventListener("input", () => config.recalcularTotal());
  item.append(rotular("Descrição", campoProduto), rotular("Quantidade", quantidadeInput), rotular("Preço unitário (R$)", preco), rotular("Subtotal", subtotal), remover);
...
async function persistirRegistro({ redirecionar = true } = {}) {
  ...
  const resposta = tabela === "orcamento"
    ? await supabaseClient.rpc("salvar_orcamento_com_itens", { p_id: id ? Number(id) : null, p_dados: campos, p_itens: itens })
    : id
  ...
  if (resposta.error)
    return avisar("Erro ao salvar: " + resposta.error.message + (tabela === "orcamento" && resposta.error.code === "PGRST202" ? " Aplique a migração de orçamentos no Supabase." : ""), true);
